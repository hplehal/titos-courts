...
export function buildDivisionBracket(division, divisionIndex) {
  ...
  return [
    ...
    // ── Week 11 SFs run IN PARALLEL on two different courts so both can
    // finish before the Final tips at 11 PM. SF1 stays on the division's
    // primary court (so seed 1 plays on the same court they QFd on); SF2
    // moves to TBD — admin assigns the actual court number once the
    // extra W11 booking is confirmed.
    {
      stage: 'sf',
      weekKey: 11,
      tierNumber: tier,
      ...
      gameOrder: 1,
      courtNumber: division.court,  // primary court for this division
    ...
    },
    {
      ...
      roundNumber: PLAYOFF_ROUND.SF,
      gameOrder: 2,
      courtNumber: null,            // TBD — extra W11 court, set by admin
      homeTeamId: s2.teamId, homeSeedLabel: `${division.name} 2`,
      awayTeamId: null,      awaySeedLabel: 'Higher QF Winner',
    },
    // ── Week 11 Final (11 PM-12 AM). Court TBD until admin confirms
    // the booking — typically a court that hosted one of this division's
    // SFs so the winning team doesn't move venues mid-night.
    {
      stage: 'final',
      weekKey: 11,
      tierNumber: tier,
