 ...
 * }>}
 */
export function computeStandingsFromMatches(teams, matches, opts = {}) {
  const rows = new Map()
  ...
  // Qualifier tags. The default (legacy gold/silver) marks top 2 → gold,
  // bottom 2 → silver. For 'crossover-single-elim' format (May 23 REC), the
  // top 3 from each pool advance directly to the bracket; positions 4 & 5
  // play a 1-set play-in for the last bracket spot — so we tag those as
  // 'playin' instead of silver. Pure presentation — no logic depends on
  // these values beyond row highlighting.
  const bracketFormat = opts.bracketFormat
  const hasResults = completed.length > 0
  if (bracketFormat === 'crossover-single-elim' && ordered.length >= 5 && hasResults) {
    ordered[0].qualifies = 'gold'
    ordered[1].qualifies = 'gold'
    ordered[2].qualifies = 'gold'
    ordered[ordered.length - 1].qualifies = 'playin'
    ordered[ordered.length - 2].qualifies = 'playin'
  } else if (ordered.length >= 4 && hasResults) {
    ordered[0].qualifies = 'gold'
    ordered[1].qualifies = 'gold'
