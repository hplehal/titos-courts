...
//
// Explicit wiring wins over both rules: when the match has nextSlot
// ('home' | 'away', set by the playoff draft or /admin/playoffs), the winner
// goes straight into that slot.
//
...
export async function advancePlayoffWinner(matchId, prisma = prismaClient) {
  ...
  // The loser of this match — used to ref the next game on the same court
  // ("the losing team in each court refs the game after").
  const loserId = match.homeTeamId && match.homeTeamId !== match.winnerId
    ? match.homeTeamId
    : (match.awayTeamId && match.awayTeamId !== match.winnerId ? match.awayTeamId : null)
  ...
  // Explicit wiring: the winner goes exactly where the admin pointed it,
  // overwriting that slot so a corrected score re-routes cleanly.
  if (match.nextSlot === 'home' || match.nextSlot === 'away') {
    await writeSlot(prisma, next.id, match.nextSlot, match.winnerId)
    const refAssigned = await assignSameCourtRef(prisma, match, next, loserId)
    return { advanced: true, nextMatchId: next.id, slot: match.nextSlot, refAssigned }
  }

  // SF → Final: SF1 winner is home, SF2 winner is away. Plus the SF loser
  // refs the Final IF the Final is on this SF's court (same-court rotation).
  if (next.roundNumber === PLAYOFF_ROUND.FINAL) {
    ...
    const refAssigned = await assignSameCourtRef(prisma, match, next, loserId)
    return { advanced: true, nextMatchId: next.id, slot, refAssigned }
  }
  ...
  if (next.roundNumber === PLAYOFF_ROUND.SF) {
    ...
    const allQfs = await prisma.match.findMany({
      ...
      select: {
        ...
        homeTeamId: true, awayTeamId: true, gameOrder: true, courtNumber: true,
      },
    ...
    const sfs = await prisma.match.findMany({
      ...
      orderBy: { gameOrder: 'asc' },
      select: { id: true, gameOrder: true, courtNumber: true, refTeamId: true },
    })
    ...
    // "Loser refs the next game on the same court." Each QF loser is
    // assigned as ref of the SF scheduled on that QF's court. Only fills
    // empty ref slots (so a manual override is never clobbered). When the
    // two QFs share a court (COED's primary-court layout), only the first
    // empty SF on that court gets a ref — harmless, since COED playoffs
    // use a certified ref. MENS splits QF1/QF2 across two courts that
    // each host one SF, so both losers land correctly.
    for (const qf of allQfs) {
      const loserId = qf.winnerId === qf.homeTeamId ? qf.awayTeamId : qf.homeTeamId
      if (!loserId || qf.courtNumber == null) continue
      const sameCourtSf = sfs.find(s => s.courtNumber === qf.courtNumber && !s.refTeamId)
      if (sameCourtSf) {
        await prisma.match.update({ where: { id: sameCourtSf.id }, data: { refTeamId: loserId } })
        sameCourtSf.refTeamId = loserId // mark locally so the second QF doesn't reuse it
      }
    }
...
// The loser refs the next match when it's on the same court and no ref is set
// yet — never clobbers a manual ref.
async function assignSameCourtRef(prisma, match, next, loserId) {
  if (!loserId || match.courtNumber == null || next.courtNumber !== match.courtNumber || next.refTeamId) return false
  await prisma.match.update({ where: { id: next.id }, data: { refTeamId: loserId } })
  return true
}
