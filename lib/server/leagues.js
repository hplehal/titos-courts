...
// League fields public pages need: identity plus the game-night rules behind
// time labels, rounds per week and playoff divisions (set on /admin/leagues).
export const PUBLIC_LEAGUE_SELECT = {
  slug: true,
  name: true,
  dayOfWeek: true,
  description: true,
  ...
  earlySlotLabel: true,
  lateSlotLabel: true,
  singleSlotLabel: true,
  timeRangeLabel: true,
}

/** Current name of an active league, for page titles (falls back to the slug). */
export async function getLeagueName(slug) {
  const leagues = await getActiveLeagues()
  return leagues.find(l => l.slug === slug)?.name || slug
}

/* ─── Homepage league cards ─── */
export const getHomeLeagues = unstable_cache(
  async () => {
    const leagues = await prisma.league.findMany({
      ...
      select: {
        ...
        description: true,
        maxTeams: true,
        defaultTierCount: true,
        seasons: {
          orderBy: { seasonNumber: 'desc' },
          take: 1,
          select: { seasonNumber: true, status: true, _count: { select: { teams: true, tiers: true } } },
        },
      },
    })
    return leagues.map(({ seasons, ...league }) => ({ ...league, season: seasons[0] || null }))
  },
  ['home-leagues'],
  { tags: ['leagues'], revalidate: 600 },
)
...
export function getLeagueStandings(slug) {
  return unstable_cache(
    async () => {
      ...
      if (!league || !league.seasons[0]) return { standings: [], currentTiers: [], divisions: [] }
      const season = league.seasons[0]
      ...
      const useHeadToHead = league.headToHead // per-league rule, on for Sunday MENS
      ...
      return { standings, currentTiers, divisions: season.divisions }
    },
