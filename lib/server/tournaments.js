...
/** Resolve a tournament's poster URL: DB-stored upload first (cheap
 * `imageType` flag), then the legacy slug-named file convention. */
function resolveTournamentImage(slug, imageType) {
  if (imageType) return `/api/tournaments/${slug}/image`
  return findTournamentImage(slug)
}
  ...
  async () => {
    ...
    return tournaments.map(t => {
      ...
      return {
        ...
        imageUrl: resolveTournamentImage(t.slug, t.imageType),
        hasLiveMatch,
      }
    })
  },
  ...
  return unstable_cache(
    async () => {
      const t = await prisma.tournament.findUnique({
        where: { slug },
        // Poster bytes are served by their own route — keep them out of this cache
        omit: { imageData: true },
        include: {
      ...
      // Attach computed standings to each pool. Pass the tournament's
      // bracketFormat so the standings table can render 'playin' qualifier
      // tags on positions 4-5 for crossover-single-elim tournaments.
      const poolsWithStandings = t.pools.map(pool => ({
        ...pool,
        standings: computeStandingsFromMatches(pool.teams, pool.matches, { bracketFormat: t.bracketFormat }),
      }))
      ...
      return {
        ...
        poolMatchFormat: t.poolMatchFormat,
        bracketMatchFormat: t.bracketMatchFormat,
